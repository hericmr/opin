  ...
  return (
    <div className={className} style={{ position: 'relative' }}>
      ...
      {isMobile ? (
        ...
        <div className="fixed top-24 left-4 w-72 bg-green-900/30 backdrop-blur-md rounded-2xl shadow-2xl z-20 overflow-hidden text-white">
          <CabecalhoMenu
            onMinimize={handleMinimize}
            isMobile={false}
            isMinimized={isMinimized}
      ...
      {isMobile ? (
        <button
          ...
          className="fixed bottom-4 right-4 z-30 rounded-full border text-green-100 border-green-100 bg-green-900/90 hover:bg-green-800 p-2.5 flex items-center cursor-pointer pointer-events-auto shadow"
        ...
        <button
          type="button"
          onClick={() => setTextScale((v) => (v >= 1.3 ? 1.0 : Math.round((v + 0.15) * 100) / 100))}
          className="fixed bottom-16 left-4 z-30 rounded-full border text-green-100 border-green-100 bg-green-900/90 hover:bg-green-800 p-2.5 flex items-center cursor-pointer pointer-events-auto shadow"
          aria-label="Ajustar tamanho do texto"
